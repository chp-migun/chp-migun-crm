...
(defun c:MMD ( / *error* osm id p1 p2 p3 org u v d3 L W tmp H
                 tN tE tS tW tC tF ops go typ p q lp lq side off w
                 hgt sill hinge cen dia zc mid rec n dtyp)
  (defun *error* (m)
  ...
  (setq tF (mg:getreal-def "עובי רצפה" 15.0))
      ...
      ((= typ "Delet")
       ...
       (if (and p q)
         (progn
           ...
           (setq w (abs (- (mg:alongwall lp side) (mg:alongwall lq side))))
           ...
           (setq hinge
             (if hinge
               (if (< (abs (- (mg:alongwall (mg:localize hinge org u v) side) off))
                      (abs (- (mg:alongwall (mg:localize hinge org u v) side) (+ off w))))
                 "A" "B") "A"))
           (setq ops (cons (list "door" side off w hgt 0.0 hinge dtyp) ops))
           (mg:check-door w hgt dtyp)
           (princ (strcat "\nדלת " (rtos w 2 0) "/" (rtos hgt 2 0)
      ...
      ((= typ "Halon")
       ...
       (if (and p q)
         (progn
           ...
           (setq ops (cons (list "win" side off w hgt sill "A") ops))
           (princ (strcat "\nחלון " (rtos w 2 0) "/" (rtos hgt 2 0)
                          " בקיר " side)))))
  ...
  (setq rec (list id L W H (list tN tE tS tW) tC tF (reverse ops)))
...
(defun mg:plan (rec ox oy th hatch? / id L W H tt tN tE tS tW tC tF ops
                s a b f side off w hgt hinge sh dir lw px py cx cy dia
                pts d1 d2 d3)
  ...
  ;; פתחים
  (foreach f ops
    (setq side (cadr f) off (caddr f) w (cadddr f) hgt (nth 4 f) hinge (nth 6 f))
    ...
    (cond
      ((= (car f) "door") (mg:dr-door px py d1 dir lw w hinge))
      ((= (car f) "win")  (mg:dr-win  px py d1 dir lw w))
      (T                  (mg:dr-vent px py d1 dir lw w))))

  ;; כיתוב מרכזי
  (mg:text "TEXT" (list (+ ox (/ L 2.0)) (+ oy (/ W 2.0) (* th 0.6))) (* th 1.5) id 1 0)
  (mg:text "TEXT" (list (+ ox (/ L 2.0)) (+ oy (/ W 2.0) (* th -1.4))) th
           (strcat (rtos (/ (* L W) 10000.0) 2 2) " \U+05DE\"\U+05E8") 1 0)
...
;;; מידות לתוכנית: שרשרת בצלע הדרומית + כוללת, פנים אנכי
(defun mg:dims (rec ox oy / L W tt tN tE tS tW ops xs pts prev x d1 d2)
  ...
  ;; פנים אנכי + כוללת
  (mg:dim nil (list (- ox tW) oy) (list (- ox tW) (+ oy W))
              (list (- ox tW d1) (+ oy (/ W 2.0))))
...
(defun mg:section (rec ox oy th hatch? / id L W H tt tE tW tC tF ops s z f)
  (setq id (nth 0 rec) L (nth 1 rec) H (nth 3 rec)
        tt (nth 4 rec) tC (nth 5 rec) tF (nth 6 rec) ops (nth 7 rec))
  (setq tE (nth 1 tt) tW (nth 3 tt))
  ;; רצפה ותקרה
  (mg:hrect "SECTION" "HATCH" (- ox tW) (- oy tF) (+ ox L tE) oy 8.0 hatch?)
  ...
  ;; מידות גובה
  (mg:dim nil (list (- ox tW) oy) (list (- ox tW) (+ oy H))
              (list (- ox tW (* 2.5 (getvar "DIMTXT") (getvar "DIMSCALE")))
                    (+ oy (/ H 2.0))))
  ;; מפלס
  (mg:pline "SECTION" (list (list (+ ox (* 0.5 L)) oy)
                            (list (+ ox (* 0.5 L) -6.0) (- oy 10.0))
                            (list (+ ox (* 0.5 L) 6.0) (- oy 10.0))) T)
  (mg:text "TEXT" (list (+ ox (* 0.5 L) 10.0) (- oy 9.0)) th "%%p0.00" 0 0)
  (mg:text "TEXT" (list (+ ox (/ L 2.0)) (- oy tF (* th 2.5))) (* th 1.2)
...
(defun c:MMDDRAW ( / *error* sv vals p0 x0 y0 th r wsum wmax gap hatch? scl
  ...
  ;; שורת חתכים מתחת
  (setq x0 (car p0) secy (- y0 (* 2.2 gap) 40.0))
  (foreach r *MMD-LIST*
  ...
  (mg:notes (+ (car p0) (* 14.0 gap)) (- y0 rowh (* 2.0 th)) th)
